...
class RobotNode(Node):
    def __init__(self):
        ...
        try:
            self.udp_socket.bind(('192.168.43.128', 12345))
            self.get_logger().info("UDP socket bound to 192.168.43.128:12345")
        ...
        self.servos = {'s1': 90, 's2': 90, 's3': 90, 's4': 90}
    ...
    def handle_encoder(self):
        if self.selected_servo and self.inputs['encoder_val'] != self.last_encoder_val:
            ...
            self.servos[self.selected_servo] = max(0, min(180, self.servos[self.selected_servo] + delta))
    ...
    def control_loop(self):
        try:
            ...
            Wz_joy = normalize_axis(self.inputs['right_x'])
